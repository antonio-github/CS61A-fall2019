...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if hasattr(insect, 'is_true_queen') and insect.is_true_queen:
                return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        range = 0
        bee = None
        while type(nearest_place) == Place or type(nearest_place) == Water:
            bee = random_or_none(nearest_place.bees)
            if isinstance(bee, Bee) and range >= self.min_range \
                                    and range <= self.max_range:
                return bee
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        self.armor -= amount
        is_dead = self.armor <= 0
        for bee in list(self.place.bees):   # Copy of self.place.bees
            # self.damage*int(is_dead) is like a step function. int(is_dead)
            # return 1 if is_dead = True and 0 if is_dead = False.
            Insect.reduce_armor(bee, amount + self.damage*int(is_dead))
        if is_dead:
            self.place.remove_insect(self)
            self.death_callback()
        # END Problem 5
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt does not attack but has a high armor for deffence."""
...
class ScubaThrower(ThrowerAnt):
    """A ScubaThrower is a ThowerAnt that is watersafe but costs more."""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        self.is_true_queen = not QueenAnt.num_queens
        QueenAnt.num_queens += 1
        ScubaThrower.__init__(self, armor)
        # END Problem 13

    def double_damage(self, place_exit):
        while place_exit:
            ant = place_exit.ant
            if ant and not getattr(ant, 'doubled_damage', False):
                ant.damage *= 2
                ant.doubled_damage = True
                if ant.is_container and ant.contained_ant:
                    ant.contained_ant.damage *= 2
                    ant.contained_ant.doubled_damage = True
