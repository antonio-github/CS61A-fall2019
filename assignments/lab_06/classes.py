...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        i, j = 0, 0
        new_player_deck, new_opponent_deck = [], []
        index_last_card_player, index_last_card_opponent = len(player.hand), len(opponent.hand)

        while i < orig_player_deck_length:
            player.draw()
            player.hand[index_last_card_player].attack += other_card.attack
            player.hand[index_last_card_player].defense += other_card.defense
            new_player_deck.append(player.hand[index_last_card_player])
            player.play(index_last_card_player)
            i += 1
        player.deck = Deck(new_player_deck)

        while j < orig_opponent_deck_length:
            opponent.draw()
            if opponent.hand[index_last_card_opponent].attack != other_card.attack and opponent.hand[index_last_card_opponent].defense != other_card.defense:
                new_opponent_deck.append(opponent.hand[index_last_card_opponent])
            opponent.play(index_last_card_opponent)
            j += 1
        opponent.deck = Deck(new_opponent_deck)
