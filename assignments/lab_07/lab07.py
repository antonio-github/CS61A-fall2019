...
def scale(s, k):
    ...
    # for item in s:
    #     yield item*k
    # # Using yield from (note that map produces a modified iterator from an
    # # existing iterator):
...
def cumulative_sum(t):
    ...
    if not t.is_leaf():
        for b in t.branches:
            cumulative_sum(b)   # Before changing t label I call recursively
                                # its branches to change their labels (from
                                # bottom to top).
...
def is_bst(t):
    ...
    if len_branches == 1:
        ...
        if bst_max(left_branch) <= t.label and bst_min(right_branch) >= t.label:
