...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        expr_evaltd = scheme_eval(expressions.first, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('uneven number of symbols and values')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('unexpected number of operators')
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(fn_name, lambda_procedure)
        return fn_name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while len(expressions) > 0:
        sub_expr = expressions.first
        expressions = expressions.rest
        eval_sub_expr = scheme_eval(sub_expr, env)
        if not scheme_truep(eval_sub_expr):
            return False
    return eval_sub_expr
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while len(bindings) > 0:
        ...
        formal = curr_binding.first
        val = scheme_eval(curr_binding.rest.first, env)
        formals, vals = Pair(formal, formals), Pair(val, vals)
    check_formals(formals)
